...
angular.module('hotelApp').controller('EmployeeDialogController',
    ['$scope', '$stateParams', '$uibModalInstance', 'entity', 'Employee', 'Gender', 'Ethnic', 'Religion', 'Job', 'Education_level', 'Major', 'School', 'Marital_status', 'Came_component', 'Bank', 'User',
        ...
        $scope.datePickerForIdentity_card_number_prov_date = {};

        $scope.datePickerForIdentity_card_number_prov_date.status = {
            opened: false
        };

        $scope.datePickerForIdentity_card_number_prov_dateOpen = function($event) {
            $scope.datePickerForIdentity_card_number_prov_date.status.opened = true;
        };
