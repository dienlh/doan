...
public class Bill_serviceResource {
     ...
     */
    @RequestMapping(value = "/bill_services",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Bill_service> createBill_service(@Valid @RequestBody Bill_service bill_service) throws URISyntaxException {
        log.debug("REST request to save Bill_service : {}", bill_service);
        if (bill_service.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("bill_service", "idexists", "A new bill_service cannot already have an ID")).body(null);
        }
        ...
        return ResponseEntity.created(new URI("/api/bill_services/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("bill_service", result.getId().toString()))
            .body(result);
    }
    ...
    @RequestMapping(value = "/bill_services",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Bill_service> updateBill_service(@Valid @RequestBody Bill_service bill_service) throws URISyntaxException {
    ...
    @RequestMapping(value = "/bill_services",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Bill_service>> getAllBill_services(Pageable pageable)
     ...
     */
    @RequestMapping(value = "/bill_services/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Bill_service> getBill_service(@PathVariable Long id) {
        ...
        return Optional.ofNullable(bill_service)
