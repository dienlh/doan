...
public class ReservationResource {
	 ...
	 */
	@RequestMapping(value = "/reservations", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<Reservation> createReservation(@Valid @RequestBody Reservation reservation)
			throws URISyntaxException {
		log.debug("REST request to save Reservation : {}", reservation);
		if (reservation.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("reservation", "idexists",
					"A new reservation cannot already have an ID")).body(null);
		}
		...
		return ResponseEntity.created(new URI("/api/reservations/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("reservation", result.getId().toString())).body(result);
	}

	/**
	 * PUT /reservations -> Updates an existing reservation.
	 */
	@RequestMapping(value = "/reservations", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	...
	public ResponseEntity<List<Reservation>> getAllReservations(Pageable pageable,
		...
		if ("bill-is-null".equals(filter)) {
			...
			return new ResponseEntity<>(reservationService.findAllWhereBillIsNull(), HttpStatus.OK);
		}
	...
	public ResponseEntity<Reservation> getReservation(@PathVariable Long id) {
		...
		return Optional.ofNullable(reservation).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
	 ...
	 */
	@RequestMapping(value = "/reservations/{id}", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	...
	@RequestMapping(value = "/reservations/findReservationNotCheckout", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
