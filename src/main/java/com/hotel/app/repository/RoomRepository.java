...
public interface RoomRepository extends JpaRepository<Room,Long> {
    ...
    @Query("select distinct room from Room room left join fetch room.images left join fetch room.funitures")
    List<Room> findAllWithEagerRelationships();

    @Query("select room from Room room left join fetch room.images left join fetch room.funitures where room.id =:id")
