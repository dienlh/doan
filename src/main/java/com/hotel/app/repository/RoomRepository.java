...
public interface RoomRepository extends JpaRepository<Room,Long> {
    ...
    @Query("select distinct room from Room room left join fetch room.images left join fetch room.amenitys")
    List<Room> findAllWithEagerRelationships();

    @Query("select room from Room room left join fetch room.images left join fetch room.amenitys where room.id =:id")
